                  ...
                  <div class="card">
                      <div class="card-body">
                          ...
                          <div class="mb-3 d-flex">
                            @if(!$company)
                            <a href="{{route('company.create')}}" class="btn primary-btn mr-2">Criar empresa</a>
                            @else
                            <a href="{{route('company.edit')}}" class="btn secondary-btn mr-2">Editar empresa</a>
                            <div class="ml-auto">
                                <form action="{{route('company.destroy')}}" id="companyDestroyForm" method="POST">
                                    @csrf
                                    @method('delete')
                                    <button type="submit" id="companyDestroyBtn" class="btn danger-btn">Deletar empresa</a>
                                </form>
                            </div>
                            @endif
                          </div>
      ...
      <section class="author-posts">
        <div class="row my-4">
          <div class="col-lg-12 col-md-8 col-sm-12">
            <div class="card">
              <div class="card-body">
                <h4 class="card-title mb-3">Gerenciar vagas</h4>
                    ...
                    <tbody>
                        @if($company)
                            ...
                            <tr>
                                ...
                                @endphp</td>
                                <td>
                                <a href="{{route('post.edit',['post'=>$post])}}" class="btn primary-btn">Editar</a>
                                <form action="{{route('post.destroy',['post'=>$post->id])}}" class="d-inline-block" id="delPostForm" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" id="delPostBtn" class="btn danger-btn">Deletar</button>
                                </form>
                                </td> 
                            </tr>
                            @endforeach
